  ...
  if (!this.svg) {
	...
	function drawAxes(axes) {
		...
		function brushed(d) {
			...
			_this.lines.selectAll(".missingpath")
				.style("stroke-opacity", (d) => {
					return _this.dataFilteringFunction(d) ? _this.defaultOpacity : _this.filteredItemOpacity;
				});
		}
	...
	function drawLines() {
		...
		let line = d3.line().defined(function(d){ return d[1] !== null ;});
		// let lineData = [];

		let paths = _this.lines.selectAll(".dataPath")
			.data(filterDataByDomains(_this.plotData));

		let missingPaths = _this.lines.selectAll(".missingpath")
			.data(filterDataByDomains(_this.plotData));

		// exit
		paths.exit().remove();
		missingPaths.exit().remove();
		...
		.attr("d", (d) => {
			...
			let points = Object.keys(axesScales).map((el, i) => {
				if(d[el] == "N/A"){
					return [(margin.left + axisSpacing * i), null]
				}else{
		...
		.attr("d", (d) => {
			...
			let filteredData = points.filter(line.defined());
			// console.log(filteredData)
			let filteredRefine = line(filteredData);
			return filteredRefine;
