// this nomogram is changed for this project purpose
...
Nomogram.prototype.draw = function() {
	...
	if (!this.plotSize) {
		// console.log("this.plotSize in null")
		...
		//changing the height for our convenient
		svgHeight = ( window.innerHeight / 2 ) - ( 2 * document.getElementById("title").clientHeight + margin.top + margin.bottom);
  ...
  if (!this.svg) {
	//   console.log("this svg is null")
      this.svg = d3.select(this.plotTarget)
          .append("svg")
          .attr("viewBox", "0 0 " + svgWidth + " " + svgHeight)
		  .attr("class", "Nomogram.svg")
		  .attr("id", "nomogram_svg");

      this.lines = this.svg.append("g");
	  this.axes = this.svg.append("g");
	  this.span = this.svg.append("span")
						   .text("hello")

      width = svgWidth;
      height = svgHeight;
  } else {
	//   console.log("this svg is not null")
	...
	function drawAxes(axes) {
		...
		// console.log(_this.plotData[5])
		let patient = _this.plotData[5];
	...
	function drawLines() {
		...
		// update
		console.log(_this.plotData)
		let keys = Object.keys(_this.plotData[5])
		for(let key of keys){
			if(_this.plotData[5][key] == "N/A"){
				for(let i = 0 ; i < _this.plotData.length - 2 ; i++){
					_this.plotData[i][key] = "N/A";
				}
			}
		}
		...
		.attr("d", (d) => {
			// console.log(d)
			let attributes = Object.keys(axesScales)
			let pointAttr = []
			...
			// let points = pointAttr.map((el,i) => {
				// console.log(el, axesScales[el]("NYN/A"))
				// console.log(axesScales)
				let previous = "" + (margin.left + axisSpacing * i) + "," + axesScales[el](d[el])
				if(d[el] == "N/A"){
					// console.log(points[i])
					if(el == "Race"){
						//take the point of gender
						return "" + (margin.left + axisSpacing * i) + "," + axesScales["Gender"](d["Gender"])
					}else{
						
						return "" + (margin.left + axisSpacing * i) + "," + axesScales["Tumor subsite (BOT/Tonsil/Soft Palate/Pharyngeal wall/GPS/NOS)"](d["Tumor subsite (BOT/Tonsil/Soft Palate/Pharyngeal wall/GPS/NOS)"])  //120
