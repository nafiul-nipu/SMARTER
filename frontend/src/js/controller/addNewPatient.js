...
let AddNewPatient = function () {
  ...
  function addNewPatient() {
    ...
    if (self.patientID == "N/A") {
      ...
      //age
      self.patientInfo[$("#age-element").attr("name")] =
        $("#age-element").val();
      self.patientInfo.AgeAtTx =
        +self.patientInfo["Age at Diagnosis (Calculated)"];
      //gender
      self.patientInfo[$("#male-radio").attr("name")] = $(
        "input:radio[name=Gender]:checked"
      ).val();
      //race
      self.patientInfo[$("#race-element").attr("name")] =
        $("#race-element").val();
      //aspiration
      self.patientInfo[$("#aspiration-y-radio").attr("name")] = $(
        "input:radio[name='Aspiration rate Pre-therapy']:checked"
      ).val();
      //hpv
      self.patientInfo[$("#hpvp16-element").attr("name")] =
        $("#hpvp16-element").val();
      //smoking status
      self.patientInfo[$("#smoking-never-radio").attr("name")] = $(
        "input:radio[name='Smoking status at Diagnosis (Never/Former/Current)']:checked"
      ).val();
      //packsper year
      if ($("#packs-per-year-element").val() == "") {
        self.patientInfo[$("#packs-per-year-element").attr("name")] = "N/A";
      } else {
        self.patientInfo[$("#packs-per-year-element").attr("name")] = $(
          "#packs-per-year-element"
        ).val();
        self.patientInfo.SmokeStatusPacksYear =
          +self.patientInfo["Smoking status (Packs/Year)"];
      }
      //tumor-site
      self.patientInfo[$("#tumor-site").attr("name")] = $("#tumor-site").val();
      //sub site
      self.patientInfo[$("#tumor-subsite").attr("name")] =
        $("#tumor-subsite").val();
      // ajcc 7th
      ...
      //t-cat
      self.patientInfo[$("#tcat1").attr("name")] = $(
        "input:radio[name='T-category']:checked"
      ).val();
      //n-cat
      self.patientInfo[$("#ncat-0").attr("name")] = $(
        "input:radio[name='N-category']:checked"
      ).val();
      //pathological grade
      self.patientInfo[$("#pgrade1").attr("name")] = $(
        "input:radio[name='Pathological Grade']:checked"
      ).val();
      //affected lymph node
      self.patientInfo[$("#affected-lymph").attr("name")] =
        $("#affected-lymph").val();
      //therapeutic
      self.patientInfo[$("#chemo-element").attr("name")] =
        $("#chemo-element").val();
      //censor
      ...
      if ($("#duration-element").val() == "") {
        ...
        self.patientInfo[$("#duration-element").attr("name")] =
          $("#duration-element").val();
        self.patientInfo.TreatmentDays =
          +self.patientInfo["Treatment duration (Days)"];
      }
      ...
      if ($("#total-dose-element").val() == "") {
        ...
        self.patientInfo[$("#total-dose-element").attr("name")] = $(
          "#total-dose-element"
        ).val();
        self.patientInfo.TotalDose = +self.patientInfo["Total dose"];
      }
      ...
      if ($("#total-fraction-element").val() == "") {
        ...
        self.patientInfo[$("#total-fraction-element").attr("name")] = $(
          "#total-fraction-element"
        ).val();
      }
      //dose element
      if ($("#dose-element").val() == "") {
        ...
        self.patientInfo[$("#dose-element").attr("name")] =
          $("#dose-element").val();
      }

      //neck dissection
      self.patientInfo[$("#neck-dissect-y-radio").attr("name")] = $(
        "input:radio[name='Neck Disssection after IMRT (Y/N)']:checked"
      ).val();
      // self.patientInfo[$('#neck-dissection-element').attr('name')] = $('#neck-dissection-element').val();
      //neck boost
      self.patientInfo[$("#neck-boost-y-radio").attr("name")] = $(
        "input:radio[name='Neck boost (Y/N)']:checked"
      ).val();
      ...
      let patient_index = App.models.patients.getPatientIDFromDummyID(
        self.patientID
      );
      ...
      //age
      if (
        Math.round(self.patientInfo[$("#age-element").attr("name")]) !=
        $("#age-element").val()
      ) {
        self.patientInfo[$("#age-element").attr("name")] =
          $("#age-element").val();
      ...
      if (
        self.patientInfo[$("#aspiration-y-radio").attr("name")] !=
        $("input:radio[name='Aspiration rate Pre-therapy']:checked").val()
      ) {
        self.patientInfo[$("#aspiration-y-radio").attr("name")] = $(
          "input:radio[name='Aspiration rate Pre-therapy']:checked"
        ).val();
      ...
      //tumoe site
      if (
        self.patientInfo[$("#tumor-site").attr("name")] !=
        $("#tumor-site").val()
      ) {
        self.patientInfo[$("#tumor-site").attr("name")] =
          $("#tumor-site").val();
      ...
      //AJCC 7th
      if (
        self.patientInfo[$("#ajcc7-1").attr("name")] !=
        $("input:radio[name='AJCC 7th edition']:checked").val()
      ) {
        self.patientInfo[$("#ajcc7-1").attr("name")] = $(
          "input:radio[name='AJCC 7th edition']:checked"
        ).val();
        self.change_made = true;
      ...
      //AJCC 8th
      if (
        self.patientInfo[$("#ajcc8-1").attr("name")] !=
        $("input:radio[name='AJCC 8th edition']:checked").val()
      ) {
        self.patientInfo[$("#ajcc8-1").attr("name")] = $(
          "input:radio[name='AJCC 8th edition']:checked"
        ).val();
        self.change_made = true;
      ...
      if (
        self.patientInfo[$("#tcat1").attr("name")] !=
        $("input:radio[name='T-category']:checked").val()
      ) {
        self.patientInfo[$("#tcat1").attr("name")] = $(
          "input:radio[name='T-category']:checked"
        ).val();
      ...
      if (
        self.patientInfo[$("#ncat-0").attr("name")] !=
        $("input:radio[name='N-category']:checked").val()
      ) {
        self.patientInfo[$("#ncat-0").attr("name")] = $(
          "input:radio[name='N-category']:checked"
        ).val();
      ...
      if (
        self.patientInfo[$("#pgrade1").attr("name")] !=
        $("input:radio[name='Pathological Grade']:checked").val()
      ) {
        self.patientInfo[$("#pgrade1").attr("name")] = $(
          "input:radio[name='Pathological Grade']:checked"
        ).val();
      ...
      if (
        self.patientInfo[$("#neck-dissect-y-radio").attr("name")] !=
        $("input:radio[name='Neck Disssection after IMRT (Y/N)']:checked").val()
      ) {
        // self.patientInfo[$('#neck-dissection-element').attr('name')] = $('#neck-dissection-element').val();
        self.patientInfo[$("#neck-dissect-y-radio").attr("name")] = $(
          "input:radio[name='Neck Disssection after IMRT (Y/N)']:checked"
      ...
      if (
        self.patientInfo[$("#neck-boost-y-radio").attr("name")] !=
        $("input:radio[name='Neck boost (Y/N)']:checked").val()
      ) {
        self.patientInfo[$("#neck-boost-y-radio").attr("name")] = $(
          "input:radio[name='Neck boost (Y/N)']:checked"
        ).val();
    ...
    if (self.change_made == true) {
      ...
      post_data["Age at Diagnosis (Calculated)"] =
        +post_data["Age at Diagnosis (Calculated)"];
      if (post_data["Smoking status (Packs/Year)"] != "N/A") {
        post_data["Smoking status (Packs/Year)"] =
          +post_data["Smoking status (Packs/Year)"];
      }
      if (post_data["Overall Survival (1=alive, 0=dead)"] != "N/A") {
        post_data["Overall Survival (1=alive, 0=dead)"] =
          +post_data["Overall Survival (1=alive, 0=dead)"];
      }
      if (post_data["Distant Control (1=no DM, 0=DM)"] != "N/A") {
        post_data["Distant Control (1=no DM, 0=DM)"] =
          +post_data["Distant Control (1=no DM, 0=DM)"];
      }
      ...
      if (post_data["Locoregional control (Time)"] != "N/A") {
        post_data["Locoregional control (Time)"] =
          +post_data["Locoregional control (Time)"];
      }
      if (post_data["Locoregional Control(1=Control,0=Failure)"] != "N/A") {
        post_data["Locoregional Control(1=Control,0=Failure)"] =
          +post_data["Locoregional Control(1=Control,0=Failure)"];
      }
      ...
      axios
        .post("https://http://qubbd-smarter.evl.uic.edu/api/backend/output", {
          patient: post_data,
